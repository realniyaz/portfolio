...
export default function SkillMatrix() {
  return (
    <section className="relative py-24 md:py-40 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto overflow-hidden">
      {/* Subtle background element - Scaled for mobile */}
      ...
      <div className="mb-16 md:mb-24">
        ...
        >
          ...
          <span className="text-white/40 tracking-[0.3em] text-[9px] md:text-[10px] font-bold uppercase">Technical Arsenal</span>
        </motion.div>
        <h2 className="text-5xl md:text-8xl font-black tracking-tighter text-white leading-[0.9] uppercase italic">
          Core <br />
          <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-white/80 to-blue-900/50">Competencies.</span>
        </h2>
      </div>
...
function SkillGroup({ id, title, skills }: { id: string; title: string; skills: string[] }) {
  ...
  return (
    ...
    >
      {/* Background reveal on interaction */}
      <motion.div 
        initial={false}
        ...
        <div className="flex flex-wrap gap-x-4 md:gap-x-6 gap-y-2 max-w-full md:max-w-md md:justify-end items-center">
          {skills.map((skill, i) => (
            <motion.span 
              ...
              className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] font-bold transition-colors whitespace-nowrap"
            >
              {skill}
            </motion.span>
          ))}
      ...
      {/* Decorative animated line - subtly colorized for tech feel */}
      <motion.div 
