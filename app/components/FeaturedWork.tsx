      ...
      >
        <div className="lg:col-span-2 p-8 md:p-12 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-white/10">
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed mb-8 md:mb-12 border-l border-blue-500/30 pl-6 font-light">{description}</p>
            <div className="flex flex-wrap gap-2">
              {tags.map((tag: string) => (
                <span key={tag} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[8px] md:text-[9px] uppercase tracking-widest text-slate-500">{tag}</span>
              ))}
            </div>
          </div>
          <Link href="/projects" className="mt-12 md:mt-16 flex items-center gap-3 text-[10px] font-bold uppercase tracking-[0.2em] text-white active:text-blue-400 lg:group-hover:text-blue-400 transition-all duration-500">
            Internal_Documentation <FiArrowUpRight />
          </Link>
        </div>

        <Link href={link} target={link.startsWith('http') ? "_blank" : "_self"} className="lg:col-span-3 h-[300px] sm:h-[400px] lg:h-auto bg-[#020617] relative flex items-center justify-center p-10 md:p-20 [perspective:1000px] cursor-pointer">
          <motion.div 
          ...
          >
            <motion.div style={{ translateZ: isMobile ? 0 : 50 }} className="absolute inset-6 md:inset-8 bg-white/[0.02] rounded-xl md:rounded-2xl border border-white/10 backdrop-blur-md flex items-center justify-center">
               <FiLayers className="text-white/5 text-7xl md:text-9xl" />
            </motion.div>
            <motion.div style={{ translateZ: isMobile ? 0 : 100 }} className="absolute -top-4 -right-4 md:-top-6 md:-right-6 p-4 md:p-8 bg-white text-black rounded-2xl md:rounded-3xl shadow-2xl">
            ...
            <motion.div style={{ translateZ: isMobile ? 0 : 150 }} className="absolute bottom-6 left-6 md:bottom-10 md:left-10 flex items-center gap-3 md:gap-4 bg-blue-600 p-3 md:p-4 rounded-full shadow-[0_0_20px_rgba(37,99,235,0.4)]">
...
function ProjectTile({ id, title, description, tags, link }: any) {
  return (
    <Link href={link} target={link.startsWith('http') ? "_blank" : "_self"} className="block">
      <motion.div 
        variants={fadeUp}
        whileTap={{ scale: 0.98 }}
      ...
      >
        ...
        <h3 className="text-2xl md:text-3xl font-black text-white mb-4 md:mb-6 uppercase tracking-tighter group-hover:text-blue-400 transition-colors italic">{title}</h3>
        <p className="text-slate-500 text-xs md:text-sm leading-relaxed mb-8 md:mb-12 font-light">{description}</p>
