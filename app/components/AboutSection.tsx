...
export default function AboutSection() {
  ...
  // Smoother spring animation for the line drawing
  const scaleY = useSpring(useTransform(scrollYProgress, [0.2, 0.6], [0, 1]), {
    stiffness: 100,
    damping: 30,
    restDelta: 0.001
  });
  ...
  return (
    <section ref={scrollRef} className="relative py-24 md:py-40 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto overflow-hidden">
      <motion.div
      ...
      >
        ...
        <div className="lg:sticky lg:top-40 flex flex-col items-center text-center lg:items-start lg:text-left">
          ...
          <motion.h2 variants={fadeUp} className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter leading-none mb-8 text-white uppercase italic">
            Engineering <br />
          ...
          <motion.p variants={fadeUp} className="text-slate-400 text-base md:text-lg leading-relaxed max-w-md border-blue-500/30 lg:border-l lg:pl-8 font-light">
            I solve complex problems by bridging the gap between <span className="text-white">high-performance architecture</span> and intuitive user psychology.
          </motion.p>

          {/* Metrics - Centered on mobile, side-by-side on desktop */}
          <motion.div variants={fadeUp} className="grid grid-cols-3 gap-6 md:gap-12 mt-12 md:mt-16 pt-10 border-t border-white/5 w-full">
        ...
        {/* RIGHT SIDE: Unique Circuit Step Animation */}
...
function ProcessStep({ icon, title, desc }: { icon: any; title: string; desc: string }) {
  return (
    ...
    >
      {/* Connector Point - Adjusted for mobile position */}
